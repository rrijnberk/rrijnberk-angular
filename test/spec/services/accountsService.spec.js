...
describe('Product overview api service', function () {
	...
	it('should return a object which has been pruned, ordered and sorted.', function () {
		httpBackend.whenGET('stub/productoverview.json').respond(mockResponse);
		accountsService.getAccounts().then(function(accounts) {
			expect(accounts.data).toEqual(expectedServiceResponse);
		});
		httpBackend.flush();
	});
